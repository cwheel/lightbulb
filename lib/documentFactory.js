...
module.exports = function(name, params, con, args) {
	...
	factory.hasMany = function(model, key) {
		if (!model.name) {
			throw new Error("'" + model.name + "' is not a valid model name, cannot add relationship to model '" + this.name + "'");
		}
		...
		factory.relationships[key] = {model: model.name, type: "hasMany"};
	}
	...
	factory.hasOne = function(model, key) {
		...
		factory.relationships[key] = {model: model.name, type: "hasOne"};
	}
